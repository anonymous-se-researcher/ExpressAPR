...
public class SideEffectAnalyzer {
    static boolean nodedup_mode = false;
    static String sidefx_db_path = null;

    final static String INJECTION_POINT_MARKER = "__TESTKIT_MARKER_SIDEFX";

    private final PreTransformer context;
    private HashMap<String, ArrayList<String>> methodToModified = new HashMap<>();

    SideEffectAnalyzer(PreTransformer context) {
        this.context = context;
    }

    public static String src_absolute_path;
    ...
    private void load_sidefx_db() {
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (!methodToModified.containsKey(items[0])) {
                    methodToModified.put(items[0], new ArrayList<>());
                }
    ...
    private void setup_symbol_solver() {
        ...
        if(src_absolute_path!=null)
            ...
            System.out.println("sidefx analyzer: src path not specified, skipped analysis for src code");
                ...
                if(!tree_skip.get()) {
                    try {
                        stmt.walk(MethodCallExpr.class, (call)->{
                            ...
                            if(ALWAYS_PURE_METHODS.contains(call.getName().toString()))
                                return; // skip if callee is known to be pure

                            if(Args.TREAT_ALL_METHODS_AS_DANGEROUS) {
                                tree_skip.set(true);
                                return;
                            }
                            ...
                            try {
                                ...
                                if (methodToModified.containsKey(signature)) {
                        ...
                        System.out.println("sidefx parsing error!");
                    }
                }
            }
